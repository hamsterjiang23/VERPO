...
from verl.utils.config import _validate_verpo_reward_kl_disabled, omega_conf_to_dataclass
...
class TestActorConfig(unittest.TestCase):
    ...
    def test_verpo_allows_only_zero_coefficient_reference_kl_switch(self):
        """RLCSD keeps the switch on at coefficient zero; VERPO still owns the anchor."""
        common = {
            "strategy": "fsdp",
            "loss_mode": "verpo_zpd",
            "use_dynamic_bsz": True,
            "rollout_n": 1,
            "verpo": VerpoZPDConfig(enabled=True),
        }

        config = ActorConfig(**common, use_kl_loss=False, kl_loss_coef=0.0)
        self.assertIsNotNone(config)

        compatibility_config = ActorConfig(
            **common, use_kl_loss=True, kl_loss_coef=0.0
        )
        self.assertTrue(compatibility_config.use_kl_loss)

        with self.assertRaisesRegex(ValueError, "owns reference anchoring"):
            ActorConfig(**common, use_kl_loss=False, kl_loss_coef=0.001)

        with self.assertRaisesRegex(ValueError, "owns reference anchoring"):
            ActorConfig(**common, use_kl_loss=True, kl_loss_coef=0.001)

    def test_generated_config_instantiates_rlcsd_snapshot_compatibility_switch(self):
        from hydra import compose, initialize_config_dir

        with initialize_config_dir(
            config_dir=os.path.abspath("verl/trainer/config"), version_base=None
        ):
            cfg = compose(
                config_name="_generated_ppo_trainer",
                overrides=[
                    "actor_rollout_ref.actor.strategy=fsdp2",
                    "actor_rollout_ref.actor.loss_mode=verpo_zpd",
                    "actor_rollout_ref.actor.use_dynamic_bsz=true",
                    "actor_rollout_ref.actor.use_kl_loss=true",
                    "actor_rollout_ref.actor.kl_loss_coef=0.0",
                    "actor_rollout_ref.actor.rollout_n=8",
                    "actor_rollout_ref.actor.verpo.enabled=true",
                    "actor_rollout_ref.actor.verpo.teacher_mode=snapshot",
                    "actor_rollout_ref.actor.verpo.teacher_sync_interval=10",
                ],
            )

        actor = omega_conf_to_dataclass(cfg.actor_rollout_ref.actor)
        self.assertIsInstance(actor, FSDPActorConfig)
        self.assertTrue(actor.use_kl_loss)
        self.assertEqual(actor.kl_loss_coef, 0.0)
        self.assertEqual(actor.verpo.teacher_mode, "snapshot")
        self.assertEqual(actor.verpo.teacher_sync_interval, 10)

    def test_verpo_rejects_reward_reference_kl(self):
        ...
        disabled = OmegaConf.create({"algorithm": {"use_kl_in_reward": False, "kl_ctrl": {"kl_coef": 0.0}}})
        _validate_verpo_reward_kl_disabled(disabled, actor_config)

        enabled_by_switch = OmegaConf.create(
            {"algorithm": {"use_kl_in_reward": True, "kl_ctrl": {"kl_coef": 0.0}}}
        )
        with self.assertRaisesRegex(ValueError, "owns reference anchoring"):
            _validate_verpo_reward_kl_disabled(enabled_by_switch, actor_config)

        enabled_by_coef = OmegaConf.create(
            {"algorithm": {"use_kl_in_reward": False, "kl_ctrl": {"kl_coef": 0.001}}}
        )
        with self.assertRaisesRegex(ValueError, "owns reference anchoring"):
            _validate_verpo_reward_kl_disabled(enabled_by_coef, actor_config)
