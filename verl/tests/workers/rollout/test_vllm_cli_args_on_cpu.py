...
class TestBuildCliArgsFromConfig:
    ...
    def test_explicit_bool_false_uses_boolean_optional_action(self):
        """Backend defaults that are true can be explicitly disabled."""
        config = {"enable_prefix_caching": False, "disable_log_stats": False}
        result = build_cli_args_from_config(config, explicit_false_flags={"enable_prefix_caching"})
