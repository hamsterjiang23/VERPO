...
class TestUidLinkedListNormalization:
    """VERPO uid fields must accept TransferQueue/tensordict LinkedList values."""

    def test_list_subclass_without_tolist(self):
        class _FakeLinkedList(list):
            pass

        values = _FakeLinkedList(["uid-a", 7])
        assert not hasattr(values, "tolist")
        assert [str(value) for value in list(values)] == ["uid-a", "7"]
