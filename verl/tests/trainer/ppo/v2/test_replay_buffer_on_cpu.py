...
def test_missing_trajectory_global_steps_is_repaired_from_prompt_metadata():
    """A partial trajectory tag inherits the authoritative prompt version."""
    rb = _make_rb(max_off_policy_strategy="drop", max_off_policy_threshold=2)
    pid = "train"
    rb.prompt_global_steps[pid]["prompt"] = 5
    batch = KVBatchMeta(
        partition_id=pid,
        keys=["prompt_0_0"],
        tags=[{"status": "success"}],
    ...
    repaired, metrics = rb._drop_max_off_policy_samples(5, pid, batch)

    assert repaired.keys == ["prompt_0_0"]
    assert repaired.tags[0]["global_steps"] == 5
    assert metrics["training/transfer_queue/repaired_missing_global_steps"] == 1


def test_unrecoverable_trajectory_global_steps_is_dropped(monkeypatch):
    """A partial tag without prompt metadata is removed instead of crashing."""
    cleared = []
    monkeypatch.setattr(tq, "kv_clear", lambda **kwargs: cleared.append(kwargs))
    rb = _make_rb(max_off_policy_strategy="drop", max_off_policy_threshold=2)
    batch = KVBatchMeta(
        partition_id="train",
        keys=["orphan_0_0"],
    ...
    repaired, metrics = rb._drop_max_off_policy_samples(5, "train", batch)

    assert repaired.keys == []
    assert metrics["training/transfer_queue/dropped_missing_global_steps"] == 1
    assert cleared == [{"partition_id": "train", "keys": ["orphan_0_0"]}]
