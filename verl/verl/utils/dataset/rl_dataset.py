...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        self.actor_prompt_suffix = str(config.get("actor_prompt_suffix", "") or "").strip()
        self.prompt_template_version = str(config.get("prompt_template_version", "") or "").strip()
        self.paper_prompt_profile = str(config.get("paper_prompt_profile", "") or "").strip()
        self.sdpg_prompt_mode = str(config.get("sdpg_prompt_mode", "") or "").strip()
        if self.sdpg_prompt_mode and self.sdpg_prompt_mode not in {"answer_only", "llm_solution"}:
            raise ValueError("sdpg_prompt_mode must be answer_only or llm_solution")
    ...
    def _build_messages(self, example: dict, key: str):
        ...
        """
        # Never mutate the Arrow/HF dataset row in-place: the same source prompt can
        # be materialized by length filtering, rollout, validation, and Teacher
        # replay.  Add the actor instruction at this one shared boundary so it is
        # present exactly once for every consumer.
        messages: list = [dict(message) for message in example[key]]
        if self.paper_prompt_profile:
            from verl.trainer.baselines.paper_prompts import apply_paper_prompt_profile

            messages = apply_paper_prompt_profile(
                messages,
                data_source=example.get("data_source"),
                profile=self.paper_prompt_profile,
            )
        if self.actor_prompt_suffix:
            user_indices = [index for index, message in enumerate(messages) if message.get("role") == "user"]
            if not user_indices:
                raise ValueError("actor_prompt_suffix requires at least one user message")
            user_index = user_indices[-1]
            content = messages[user_index].get("content", "")
            if not isinstance(content, str):
                raise TypeError("actor_prompt_suffix requires a string user-message content")
            base = content.rstrip()
            while base.endswith(self.actor_prompt_suffix):
                base = base[: -len(self.actor_prompt_suffix)].rstrip()
            messages[user_index]["content"] = f"{base}\n\n{self.actor_prompt_suffix}"
    ...
    def __getitem__(self, item):
        ...
        if self.sdpg_prompt_mode:
            from sdpg_reproduction.src.sdpg_prompt import (
                SDPGPromptConfig,
                render_sdpg_messages,
                resolve_sdpg_solution,
            )
            reward_model = row_dict.get("reward_model") or {}
            answer = reward_model.get("ground_truth") if isinstance(reward_model, dict) else None
            answer = answer if answer is not None else row_dict.get("answer")
            raw_prompt = render_sdpg_messages(
                raw_prompt,
                answer=answer,
                solution=resolve_sdpg_solution(row_dict),
                config=SDPGPromptConfig(mode=self.sdpg_prompt_mode),
            )
