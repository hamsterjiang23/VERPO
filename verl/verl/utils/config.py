...
def _validate_verpo_reward_kl_disabled(config: DictConfig, actor_config: Any) -> None:
    """Keep VERPO's reference anchor exclusive from reward-side reference KL."""
    if not actor_config.verpo.enabled:
        return

    reward_kl_coef = float(OmegaConf.select(config, "algorithm.kl_ctrl.kl_coef", default=0.0))
    if config.algorithm.get("use_kl_in_reward", False) or reward_kl_coef != 0.0:
        raise ValueError(
            "VERPO-ZPD owns reference anchoring through actor.verpo.lambda_ref; "
            "set algorithm.use_kl_in_reward=false and algorithm.kl_ctrl.kl_coef=0"
        )
...
def validate_config(
    ...
    _validate_verpo_reward_kl_disabled(config, actor_config)
