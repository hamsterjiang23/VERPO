...
class Tracking:
    ...
    def finish(self, exit_code: int = 0) -> None:
        """Flush and close every tracking backend exactly once."""
        if getattr(self, "_finished", False):
            return
        self._finished = True

        for backend, logger_instance in getattr(self, "logger", {}).items():
            try:
                if backend in {"wandb", "vemlp_wandb"}:
                    logger_instance.finish(exit_code=exit_code)
                elif backend not in {"console", "mlflow"}:
                    logger_instance.finish()
            except Exception:
                logger.exception("Failed to finish %s tracking backend", backend)

    def __del__(self):
        # Explicit trainer shutdown is authoritative. This remains only as a
        # best-effort fallback for callers that have not adopted finish().
        try:
            self.finish()
        except Exception:
            pass
