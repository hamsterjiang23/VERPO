...
class DAPORewardManager(RewardManagerBase):
    ...
    async def run_single(self, data: DataProto) -> dict:
        ...
        for context_key in ("uid", "session_id"):
            if context_key not in data_item.non_tensor_batch:
                continue
            context_value = data_item.non_tensor_batch[context_key]
            if hasattr(context_value, "item"):
                context_value = context_value.item()
            extra_info.setdefault(context_key, context_value)
