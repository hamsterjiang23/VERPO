...
class SingleTurnAgentLoop(AgentLoopBase):
    ...
    async def run(self, sampling_params: dict[str, Any], priority: int = 0, **kwargs) -> AgentLoopOutput:
        ...
        from sdpg_reproduction.src.sdpg_prompt import student_visible_messages

        messages = student_visible_messages(list(kwargs["raw_prompt"]))
        ...
        effective_response_length = int(sampling_params.get("max_tokens", self.response_length))
        ...
        output: AgentLoopOutput = AgentLoopOutput(
            ...
            response_logprobs=response_logprobs[:effective_response_length] if response_logprobs else None,
            routed_experts=(
        ...
        output.extra_fields["effective_response_length"] = effective_response_length
