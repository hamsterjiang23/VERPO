...
class ReplayBuffer:
    ...
    def _drop_max_off_policy_samples(
        ...
        drop_stale = self.max_off_policy_strategy == "drop"
        kept_keys, kept_tags = [], []
        dropped_keys, dropped_tags = [], []
        repaired_missing_global_steps = 0
        dropped_missing_global_steps = 0
        for key, tag in zip(batch.keys, batch.tags, strict=False):
            prompt_global_steps = tag.get("global_steps")
            if prompt_global_steps is None:
                prompt_uid = key.split("_")[0]
                prompt_global_steps = self.prompt_global_steps[partition_id].get(prompt_uid)
                if prompt_global_steps is None:
                    dropped_keys.append(key)
                    dropped_tags.append(tag)
                    dropped_missing_global_steps += 1
                    continue
                # KVBatchMeta tags are ordinary dictionaries.  Repair the
                # sampled copy so every downstream consumer sees the same
                # authoritative prompt version rather than a partial tag.
                tag = dict(tag)
                tag["global_steps"] = prompt_global_steps
                repaired_missing_global_steps += 1
            if drop_stale and (
                (global_steps - prompt_global_steps + 1) / self.parameter_sync_step
        ...
        metrics = {}
        if repaired_missing_global_steps:
            metrics[f"{prefix}/transfer_queue/repaired_missing_global_steps"] = repaired_missing_global_steps
        if dropped_missing_global_steps:
            metrics[f"{prefix}/transfer_queue/dropped_missing_global_steps"] = dropped_missing_global_steps
        if len(dropped_keys) > 0:
            ...
            stale_tags = [tag for tag in dropped_tags if tag.get("global_steps") is not None]
            metrics[f"{prefix}/off_policy/dropped_samples"] = len(dropped_keys)
            if stale_tags:
                dropped_global_steps = np.array([tag["global_steps"] for tag in stale_tags])
        ...
        if repaired_missing_global_steps:
            logger.warning(
                "Repaired %s trajectory tags without global_steps from prompt metadata in partition %s",
                repaired_missing_global_steps,
                partition_id,
            )
        if dropped_missing_global_steps:
            logger.error(
                "Dropped %s trajectory tags without recoverable global_steps in partition %s",
                dropped_missing_global_steps,
                partition_id,
            )
