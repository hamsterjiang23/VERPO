...
class AgentLoopWorkerTQ(AgentLoopWorker):
    def __init__(self, *args, **kwargs):
        ...
        # TransferQueue's async client owns a request socket per worker.  A
        # single AgentLoopWorker runs many prompt/session coroutines at once,
        # but request sockets are not safe for overlapping multipart writes.
        # Serialize only the short enqueue operation; generation and Teacher
        # scoring remain fully concurrent.
        self._transfer_queue_put_lock = asyncio.Lock()

    async def _transfer_queue_put(self, **kwargs) -> None:
        async with self._transfer_queue_put_lock:
            await tq.async_kv_put(**kwargs)

    async def _transfer_queue_batch_put(self, **kwargs) -> None:
        async with self._transfer_queue_put_lock:
            await tq.async_kv_batch_put(**kwargs)
    ...
    async def generate_sequences(self, batch: TensorDict) -> None:
        ...
        if validate:
            ...
            validation_response_length = (config.custom or {}).get(
                "val_response_length", config.response_length
            )
            sampling_params["max_tokens"] = int(validation_response_length)
    ...
    async def _run_prompt(self, prompt: dict, sampling_params: dict, trajectory: dict, trace: bool = False) -> None:
        ...
        await self._transfer_queue_put(key=uid, partition_id=partition_id, tag={"status": "running"})
        try:
            ...
            await self._transfer_queue_put(key=uid, partition_id=partition_id, tag={"status": "finished"})
        except Exception as e:
            logger.exception(f"Error in _run_prompt: {e}")
            await self._transfer_queue_put(key=uid, partition_id=partition_id, tag={"status": "failure"})
    ...
    async def _agent_loop_postprocess(
        ...
        await self._transfer_queue_batch_put(
            keys=keys,
