...
def _serialize_lora_tensors_portably(tensors: dict[str, torch.Tensor]) -> str:
    """Serialize LoRA weights without multiprocessing file-descriptor handles.

    SGLang's ``MultiprocessingSerializer`` uses ``ForkingPickler``.  That is
    efficient inside one multiprocessing family, but dynamic LoRA updates cross
    independent Ray and SGLang process trees over HTTP.  Tensor storage handles
    created by the sender then fail authentication in the scheduler process.

    A regular pickle embeds the CPU tensor storage in the request payload.  The
    resulting base64 string remains compatible with SGLang's existing
    ``MultiprocessingSerializer.deserialize`` endpoint.
    """
    cpu_tensors = {
        name: tensor.detach().to(device="cpu").contiguous() for name, tensor in tensors.items()
    }
    payload = pickle.dumps(cpu_tensors, protocol=pickle.HIGHEST_PROTOCOL)
...
class ServerAdapter(BaseRollout):
    ...
    def __init__(
        ...
        #   2 = release weights + kv_cache (full sleep, merge/non-LoRA path)
        #   1 = release kv_cache only (keep base weights, adapter path)
        #
        # The HTTP server selects the same policy before the first actor weight
        # sync.  Initialize it here as well so update_weights() never tries to
        # resume a ``weights`` tag that adapter-mode SGLang did not release.
        lora_as_adapter = (
            self.model_config.lora_rank > 0 or self.model_config.lora.get("rank", 0) > 0
        ) and not self.model_config.lora.get("merge", False)
        self.sleep_level = 1 if lora_as_adapter else 2
    ...
    def wrap_lora_params(self, peft_config: LoraConfig, weights: Generator[tuple[str, torch.Tensor]]):
        ...
        if peft_config_json.get("target_modules") is not None:
        ...
        serialized_tensors = _serialize_lora_tensors_portably(processed_weights)
