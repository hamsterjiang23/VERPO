...
VLLM_LORA_NAME = "123"
VLLM_LORA_PATH = os.getenv("VERL_VLLM_LORA_PATH", "simon_lora_path")
...
def build_cli_args_from_config(
    config: dict[str, Any], *, explicit_false_flags: set[str] | frozenset[str] = frozenset()
) -> list[str]:
    """
    ...
    - None: skipped
    - bool True: adds '--key'
    - bool False: skipped, unless the key is listed in ``explicit_false_flags``;
      those keys emit vLLM's ``--no-...`` BooleanOptionalAction spelling
    ...
    Args:
        ...
        explicit_false_flags: BooleanOptionalAction keys whose false value must
            override a backend default of true.
    ...
    for k, v in config.items():
        ...
        if isinstance(v, bool):
            if v:
                cli_args.append(f"--{k}")
            elif k in explicit_false_flags:
