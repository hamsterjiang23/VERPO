...
class vLLMHttpServer:
    ...
    async def launch_server(self, master_address: str = None, master_port: int = None, dp_rpc_port: int = None):
        ...
        # vLLM V1 defaults prefix caching to enabled.  Omitting a false boolean
        # therefore silently re-enables it, so serialize this particular
        # BooleanOptionalAction explicitly as --no-enable-prefix-caching.
        server_args = ["serve", self.model_config.local_path] + build_cli_args_from_config(
            args, explicit_false_flags={"enable_prefix_caching"}
        )
