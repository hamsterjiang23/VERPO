...
def modulate_advantages(
    ...
    """Scale advantage magnitude with detached controller weights."""
    if advantages.shape != weights.shape:
        raise ValueError("advantages and weights must have identical shapes")
    scale = 1.0 + float(coefficient) * weights.detach()
    if not bool(torch.isfinite(scale).all().item()):
        raise ValueError("advantage modulation scale must be finite")
...
def ppo_loss(
    ...
    advantages_override: torch.Tensor | None = None,
):
    """Computes ppo loss from model output (log_prob, entropy, values, etc. ) and old_log_probs from data."""
    log_prob = no_padding_2_padding(model_output["log_probs"], data)
    reference_log_probs_padded = (
        no_padding_2_padding(reference_log_probs, data) if reference_log_probs is not None else None
    )
    ...
    if reference_log_probs is None and "ref_log_prob" in data:
    ...
    if advantages_override is not None:
        if advantages_override.shape != advantages.shape:
            raise ValueError(
                "advantages_override must have the same padded shape as advantages"
            )
        advantages = advantages_override.to(device=advantages.device, dtype=advantages.dtype)
    ...
    if config.use_kl_loss:
        ref_log_prob = (
            reference_log_probs_padded
            if reference_log_probs_padded is not None
            else data["ref_log_prob"]
        )
        # compute kl loss
