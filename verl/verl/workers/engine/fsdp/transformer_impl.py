...
def _get_vision_tower(module):
    """Return a vision submodule through common Hugging Face/PEFT wrappers."""
    pending = [module]
    visited = set()
    while pending:
        ...
        for attr in ("visual", "vision_tower"):
            vision_tower = getattr(candidate, attr, None)
...
class FSDPEngine(BaseEngine):
    ...
    def _build_lora_module(self, module):
        ...
        if os.getenv("VERL_DISABLE_PEFT_TORCHAO_DISPATCH") == "1":
            torchao_weights = [
                name
                for name, submodule in module.named_modules()
                if (weight := getattr(submodule, "weight", None)) is not None
                and type(weight).__module__.startswith("torchao")
            ]
            if torchao_weights:
                raise RuntimeError(
                    "VERL_DISABLE_PEFT_TORCHAO_DISPATCH=1 cannot be used with TorchAO-backed weights: "
                    + ", ".join(torchao_weights[:5])
                )
            from peft.tuners.lora import torchao as peft_torchao

            peft_torchao.is_torchao_available = lambda: False
            logger.info("Disabled optional PEFT TorchAO LoRA dispatch for standard dense weights")
    ...
    def _build_model_optimizer(self):
        ...
        if self.model_config.freeze_vision_tower:
            vision_tower = _get_vision_tower(module)
            if vision_tower is None:
                logger.warning("freeze_vision_tower=true, but the model has no recognized vision tower")
            else:
                vision_tower.requires_grad_(False)
                logger.info("Vision tower parameters are frozen")
    ...
    def optimizer_step(self):
        ...
        if scaler is not None:
            ...
            scaler.update()
            self._last_optimizer_step_applied = float(scaler.get_scale()) >= previous_scale
        else:
...
class FSDPEngineWithLMHead(FSDPEngine):
    ...
    def prepare_model_outputs(
        self, output, output_args, micro_batch: TensorDict, logits_processor_func
    ):
        use_remove_padding = tu.get_non_tensor_data(data=micro_batch, key="use_remove_padding", default=True)
        pad_mode = tu.get_non_tensor_data(data=micro_batch, key="pad_mode", default=DatasetPadMode.NO_PADDING)
        use_fused_kernels = tu.get_non_tensor_data(data=micro_batch, key="use_fused_kernels", default=False)
        ...
        verpo_use_full_logits = tu.get_non_tensor_data(
            data=micro_batch, key="verpo_use_full_logits", default=False
        )
        actor_loss_use_full_logits = tu.get_non_tensor_data(
            data=micro_batch, key="actor_loss_use_full_logits", default=False
        )
        use_logits_processor = (
            distillation_use_topk
            or verpo_use_full_logits
            or actor_loss_use_full_logits
        )

        if (verpo_use_full_logits or actor_loss_use_full_logits) and use_fused_kernels:
            raise NotImplementedError(
                "The selected actor loss requires eager full logits; "
                "set actor.use_fused_kernels=False"
            )
    ...
    def forward_step(self, micro_batch: TensorDict, loss_function, forward_only):
        ...
        with autocast_ctx:
            monitor_verpo = "verpo_evidence_rollout_gate" in micro_batch
            student_forward_started = time.perf_counter() if monitor_verpo else 0.0
            ...
            if monitor_verpo:
                tu.assign_non_tensor_data(
                    micro_batch,
                    "verpo_student_forward_time_seconds",
                    time.perf_counter() - student_forward_started,
                )
